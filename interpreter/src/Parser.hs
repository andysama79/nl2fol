...
markIndividualSpacing s = markIndividualSpacing' s False
  where
    ...
    markIndividualSpacing' (c : s) quoted = case c of
      '\"' ->
        if quoted
          then c : markIndividualSpacing' s False
          else c : markIndividualSpacing' s True
      ' ' ->
        if quoted
          then '_' : markIndividualSpacing' s quoted
  ...
  where
    ...
    xs =
      map
        ( \s ->
            if '\"' `elem` s
              then (removeExcessSpaces . init . tail) s
...
parseQuantifier s = \x -> parse s''
  where
    ...
    replaceInd (Pred name xs) old new =
      Pred
        name
    ...
    replaceInd (Conj p1 p2) old new =
      Conj
        (replaceInd p1 old new)
        (replaceInd p2 old new)
    replaceInd (Impl p1 p2) old new =
      Impl
        (replaceInd p1 old new)
        (replaceInd p2 old new)
